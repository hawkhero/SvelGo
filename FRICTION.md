...
**Resolution:** Implemented `cmd/svelgo/main.go` — a stdlib-only CLI binary (`os`, `os/exec`, `text/template`) with a `svelgo new <appname>` subcommand. Running `go run ./cmd/svelgo new myapp` (from inside `<repo>/demo/` or any directory one level below the framework root) creates all 10 scaffold files, runs `go mod tidy` in the new app directory, and runs `npm install` in the new `frontend/` directory. The generated app builds and runs immediately with `make dev`. Error handling: exits 1 if the directory already exists, if `go mod tidy` fails, or if `npm install` fails (files left in place for debugging). The generated `go.mod` replace directive points to `../../` and the `svelgo` npm path points to `../../../frontend`, matching the convention used by `demo/clickcounter/` (two levels of nesting below the framework root). The stdout output includes a note explaining the path assumption and how to adjust for different nesting depths. · 2026-04-12
